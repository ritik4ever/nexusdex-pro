...
import { usePriceStore, okxPriceService } from "@/lib/api/okx-price-service"
...
export function LivePriceDashboard() {
  ...
  useEffect(() => {
    ...
    return () => okxPriceService.disconnect()
  }, [])
  ...
  const totalMarketCap = Object.values(prices).reduce((sum, price) => sum + price.marketCap, 0)
  const totalVolume = Object.values(prices).reduce((sum, price) => sum + price.volume24h, 0)
  const avgChange =
    Object.values(prices).reduce((sum, price) => sum + price.changePct24h, 0) / Object.values(prices).length
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        <motion.div
          ...
          className="premium-glass p-6 text-center group hover:scale-105 transition-all duration-300"
        >
          <div className="text-4xl font-bold neon-text mb-2">${(totalMarketCap / 1e12).toFixed(1)}T</div>
          <div className="text-sm text-gray-400 uppercase tracking-wider">Total Market Cap</div>
        ...
        <motion.div
          ...
          className="premium-glass p-6 text-center group hover:scale-105 transition-all duration-300"
        >
          <div className="text-4xl font-bold text-green-400 mb-2">${(totalVolume / 1e9).toFixed(1)}B</div>
        ...
        <motion.div
          ...
          className="premium-glass p-6 text-center group hover:scale-105 transition-all duration-300"
        >
          <div className={`text-4xl font-bold mb-2 ${avgChange >= 0 ? "text-green-400" : "text-red-400"}`}>
            {avgChange >= 0 ? "+" : ""}
            {avgChange.toFixed(1)}%
          </div>
          <div className="text-sm text-gray-400 uppercase tracking-wider">Market Trend</div>
          <div
            className={`w-full h-1 rounded-full mt-3 opacity-50 group-hover:opacity-100 transition-opacity ${
              avgChange >= 0
        ...
        <motion.div
          ...
          className="premium-glass p-6 text-center group hover:scale-105 transition-all duration-300"
        >
          <div className="text-4xl font-bold text-purple-400 mb-2">{Object.keys(prices).length}</div>
          <div className="text-sm text-gray-400 uppercase tracking-wider">Live Assets</div>
      ...
      {/* Live Market Data */}
      <Card className="premium-glass border-white/20">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-gray-400 mt-2 flex items-center gap-2">
                  ...
                  Real-time prices from OKX API • Updated every 10 seconds
                </div>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div
                className={`w-3 h-3 rounded-full ${isConnected ? "status-online animate-pulse" : "status-offline"}`}
              />
              <Badge
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {marketData.map((token, index) => (
              <motion.div
                ...
                className="premium-glass p-6 group hover:scale-105 hover:border-cyan-500/30 transition-all duration-300"
              >
